...
def holdout(model, xFeat, y, testSize):
    ...
    yTrainScore = model.predict_proba(xTrain)[:, 1]
    yTestScore = model.predict_proba(xTest)[:, 1]

    trainAuc = metrics.roc_auc_score(yTrain, yTrainScore)
    testAuc = metrics.roc_auc_score(yTest, yTestScore)
...
def mc_cv(model, xFeat, y, testSize, s):
    ...
    mc = ShuffleSplit(n_splits=s, test_size = testSize, random_state=1)
