...
def add_irr_feature(xTrain, xTest):
    ...
    """
    # TODO FILL IN
    noise1 = pd.DataFrame(np.random.randn(2, len(xTrain.columns)), columns=xTrain.columns)
    noise2 = pd.DataFrame(np.random.randn(2, len(xTest.columns)), columns=xTest.columns)
    xTrain = xTrain.append(noise1, ignore_index=True)
    xTest = xTest.append(noise2, ignore_index=True)
